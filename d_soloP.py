...
def likelihood(args):
    ...
    d_tmp = alladd(d_phif0)
    m_tmp = np.average(alladd(m_phif0))
    print(wt.shape, np.log(d_tmp).shape, np.log(m_tmp))
...
b2 = e1
e2 = b2 + 100000
data_phif0,data_phi,data_f = mcnpz(0,1000000)
mc_phif0,mc_phi,mc_f = mcnpz(0,1000000)
wt = weight(wtarg)
likelihood(wtarg)
...
for j in range(10):
    start = time()
    print(grad_likelihood(wtarg))
    print('grad time:', float(time() - start))
